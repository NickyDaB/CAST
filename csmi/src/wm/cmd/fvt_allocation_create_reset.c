...
static void help() {
	puts("_____CSM_FVT_ALLOCATION_CREATE_RESET_CMD_HELP_____");
	puts("USAGE:");
	puts("  fvt_allocation_create_reset ARGUMENTS [OPTIONS]");
	puts("  fvt_allocation_create_reset -j primary_job_id -n \"node01,node02\" [-J secondary_job_id] [-s state] [-t type] [-u user_name] [-U user_id] [-h] [-v verbose_level] [--create_cgroup] [-l launch_node_name]");
	puts("");
	puts("SUMMARY: Used to create an allocation. - Reset to a state that requires a core blink. - not mess up statistical numbers for allocation create doing a core blink.");
	puts("");
	puts("EXIT STATUS:");
	puts("  0  if OK,");
	puts("  1  if ERROR.");
	...
	puts("  fvt_allocation_create_reset -j 1 -n \"node01\"");
